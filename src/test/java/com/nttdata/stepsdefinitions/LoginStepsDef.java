...
public class LoginStepsDef {
    ...
    public void agrego_un_item_al_carrito_de_compras() {
        ...
        inventorySteps.addToCartItem();
        inventorySteps.viewCart();
        screenShot();
    }
    ...
    public void reviso_que_el_carrito_de_compras_tenga_un_item_agregado() {
       int itemsCartListSize = cartSteps(driver).getCartSize();
       screenShot();
       Assertions.assertTrue(itemsCartListSize > 0, "El tamaño de la lista es: " + itemsCartListSize);
    }
    @Y("valido que el item agregado se llame {string}")
    public void valido_que_el_item_agregado_se_llame(String productTitle) {
        String title =  cartSteps(driver).getTitle();
        //prueba: validamos el título del producto
        Assertions.assertEquals(productTitle, title);
        screenShot();
    }
